...
protected func Initialize()
{
  ...
  CreateContents(ReturnRandomID()); Initialized();
}
...
protected func Timer()
{
  ...
  if(!Contents())
    ...
    initialized = false;
	if(!GetEffect("NoContents",this())) CreateContents(ReturnRandomID());
    return();
    }
...
public func RejectEntrance(object pClonk)
{
  ...
  if(CheckCollect(GetOwner(pClonk)))
  {
    ...
    // Kann der Clonk einsammeln?
    if(ObjectCount(0, 0, 0, 0, 0, 0, 0, 0, pClonk) >= 3)
      return(1);
    ...
    else
    {
	  ...
	  AddEffect("NoContents",this(),100,350,this());
    }
  }
  return(1);
}
...
func FxNoContentsTimer() { return(-1); }

private func ReturnRandomID() {
 var aObjects=[WOOD,WOOD,WOOD,WOOD,METL,METL,METL,METL,SPHR,SPHR,SPHR,SPHR,ASHS,ASHS,ASHS,ASHS,ASHS,LBRL,WBRL,WBRL,OBRL,ABRL,EFLN,EFLN];
 return(aObjects[Random(GetLength(aObjects))]);
 }